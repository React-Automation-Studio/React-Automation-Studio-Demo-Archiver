...
do
	...
	# echo "Setting MYSQL_CONNECTION_STRING to ${MYSQL_CONNECTION_STRING}"
	sleep ${seconds}
	mysql --host=127.0.0.1 --user=root --password=archappl -e "CREATE DATABASE IF NOT EXISTS archappl"
	numtries=numtries+1
	#mysql ${MYSQL_CONNECTION_STRING} -e "SHOW DATABASES" | grep information_schema
done
MSG="successfully connected after ${numtries} tries to  MySQL connection string ${MYSQL_CONNECTION_STRING}"
	echo ${MSG}
